...
HP ProBook 6540b имел инновационную возможность серфинга и просмотра электронной почты без загрузки основной ОС, для экономии питания при автономной работе. Т.н. QuickWeb и  QuickLook. То есть, в прошивке есть драйвера для WiFi карт из whitelist. Можно предположить, для карты, отсутствующей в официальном листе, эта функциональность работать не будет, и, вероятно, при включении этих опций в BIOS вообще может окирпичится.

Для начала неплохо бы понять - а что вообще входит в белый лист и как он устроен. 
...
1. Взятый из распакованной прошивки HP update **BIOS 68CDD rev.F60** открываю в ```010 Editor```.  (в репозитории **/src/BIOS/68CDD.Ver.F.60 hp.BIN**)
...
4. Создаю новый темплейт (на предыдущем скриншоте он уже присутствует) с содержимым листинга ниже. C-like синтаксис, проблем с пониманием не должно быть.
		...
		typedef struct{
			...
			WORD mb_rev;		// A, B или F. Используется внутри исполняемых модулей как параметр инициализации
		...
		WL_WIFI wifi[16];	//вот он, массив элементов whitelist

 Применить &lt;F5&gt; к открытому файлу BIOS.
 
 ![010editor wl equipment](/pix/2021-03-03_11-23-36.png)
 
5. Структура WL_WIFI - эмпирическая, слепленная мной "на глаз", предназначение **mb_rev** мне не понятно, принимает значения A, B или F. Используется внутри исполняемых модулей как параметр инициализации, но сильно глубоко не копал.

Количество записей в **WL_WIFI wifi[16]** ровно так же - на глаз.  

Результат - список оборудования. На скрине - предварительная, старая версия темплейта, новая **whitelist_equipment.bt ** в [/src](\src\whitelist_equipment.bt) репозитория.

Там изменено формирование строки структуры 
			
Чтобы скопировав (right click - Copy column) колонку результатов Value получилась почти MarkDown - таблица, к которой осталось только приделать шапку, распознать модели и написать заметки в правом столбце
...
Вот на место самой первой, **Centrino Advanced-N 6200 2x2 AGN**, и буду прописывать параметры для новой **AC 7265**.
...
Intel Corp уверен, что AC7265, в отличие от AC7260, выпускается [исключительно](https://www.intel.com/content/dam/www/public/us/en/documents/product-briefs/dual-band-wireless-ac-7265-brief.pdf) в форм-факторах платы ```M,2 2230``` и ```M.2 1216```.

Однако вот у меня на руках карта в формате```mPCIe```, полученная с алиэкспресс. Linux Mint 19 считает вставленную в ноутбук ASER к53 (без whitelist в BIOS)  именно ```Intel® Dual Band Wireless-AC 7265```.
...
### Зачем так глубоко копаться.

В первоначальной версии этого раздела не было. После изменения и прошивки BIOS, оказалось, что BlueTooth на карте - не виден в устройствах на HP ProBook (но виден на ASER без whitelist).
